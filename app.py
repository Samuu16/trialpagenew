...
def receive_level_sensor_data():
    if request.method == 'POST':
        try:
            ...
            if not sense_data:
                raise ValueError('modbus_TEST key is missing in the JSON data')
            ...
            # Log the exception with the incoming data
            logging.error('Failed to process level sensor data: %s', request.data, exc_info=True)

            # Return an error response
            response = {'status': 'error', 'message': 'Failed to process data'}
